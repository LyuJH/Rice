...
cv::Mat process(cv::Mat &input_image,
  ...
  for (int i = 0; i < results.size(); i++) {
    std::cout << "image size: " << input_image.cols << "x"
                    << input_image.rows
                    << ", detect object: " << results[i].class_name.c_str()
                    << ", score: " << results[i].score << ", locationx:" << ((results[i].right + results[i].left) / 2) * input_image.cols
                    << ", locationy: " << ((results[i].bottom + results[i].top) / 2) * input_image.rows << ", width:" << (results[i].right - results[i].left) * input_image.cols << ", height:" << (results[i].bottom - results[i].top) * input_image.rows
                    << std::endl;
...
int main(int argc, char** argv) { 
  ...
  std::string label_path = "/opt/deploy/label_list";
  //std::string image_path = "/opt/steel_images/47.jpg";
  ...
  cxx_config.set_valid_places({
    paddle::lite_api::Place{TARGET(kIntelFPGA), PRECISION(kFloat)},
    paddle::lite_api::Place{TARGET(kARM), PRECISION(kFloat)}
  });

  // Create Predictor For Detction Model
  predictor = CreatePaddlePredictor<CxxConfig>(cxx_config);
  int start = image_path.find_last_of("/");
  int end = image_path.find_last_of(".");
  std::string img_name = image_path.substr(start + 1, end - start - 1);
  //std::string result_name = "./output/"+img_name+".jpg";
  std::string output_image_path = "./output/" + img_name + ".jpg";
